...
import { MessageSquare, BookOpen, Map as MapIcon, ArrowRight, Activity, Users } from "lucide-react";
    ...
    <div className="container max-w-2xl px-4 py-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Link href="/chat" className="block w-full">
          <Card className="h-full hover-elevate transition-all border-primary/20 bg-primary/5">
            <CardHeader>
        ...
        <Link href="/diagnostico" className="block w-full">
          <Card className="h-full hover-elevate transition-all border-secondary/20 bg-secondary/5">
            <CardHeader>
        ...
        <div className="flex flex-wrap gap-2">
          {topicsLoading ? (
            ...
            popularTopics?.map((topic) => (
              <Link key={topic.topic} href={`/chat?q=${encodeURIComponent(topic.topic)}`}>
                <div className="inline-flex items-center rounded-full border px-4 py-2 text-sm font-medium transition-colors hover:bg-muted focus:outline-none">
                  {topic.topic}
                </div>
              </Link>
            ))
